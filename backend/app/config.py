...
- When asked about available portfolio use get_deals_data with 'portfolioquery' from config.
...
"""This query retrieves detailed trade information for a specific portfolio, focusing on trades related to future gas. 
It combines data from multiple sources, including trade headers, trade legs, trade profiles, and profit and loss details. The results include key trade attributes such as counterparty, trade date, trader, instrument type, and financial metrics like realized and unrealized values. 
The output is limited to the first 50 entries that meet the specified criteria. 
entity_trade_header
entity_trade_leg
...
WITH header_leg AS (
  SELECT
    ...
    h.internal_portfolio AS portfolio_name,
    h.trade_status,
    h.trade_currency,
    h.trade_price,
